...
export default function AdminSermonsScreen() {
  ...
  const { items: sermons, loading } = useAdminList('sermons', {
        ...
        ListEmptyComponent={
          !loading ? (
            <View style={styles.centered}>
              <Text style={theme.typography.body}>No sermons yet. Add the first one.</Text>
            </View>
          ) : null
        }
        renderItem={({ item }) => (
          <Card onPress={() => router.push(`/admin/sermons/${item.id}`)}>
            <Text style={theme.typography.heading}>{item.title}</Text>
            <Text style={theme.typography.caption}>
              {new Date(item.sermon_date).toLocaleDateString()}
              {item.speaker ? ` • ${item.speaker}` : ''}
            </Text>
          </Card>
        )}
      />
      <Pressable style={styles.fab} onPress={() => router.push('/admin/sermons/new')}>
